...
export function registerBigQueryTools(server, service, config) {
    server.tool("list_tables", "List all tables and views in the revival_ai_data dataset with their descriptions", {}, async () => {
        try {
    ...
    server.tool("get_table_schema", "Get the full schema (columns, types, descriptions) for a specific table", { table_id: z.string().describe("The table or view name within revival_ai_data") }, async ({ table_id }) => {
        try {
    ...
    server.tool("query", "Run a read-only SQL query against the revival_ai_data dataset.", {
    ...
    server.tool("preview_table", "Preview the first N rows of a table (default 10)", {
